...
class OFChannelHandler extends SimpleChannelInboundHandler<Iterable<OFMessage>> {
    ...
    class WaitHelloState extends OFChannelState {
        ...
        void processOFHello(OFHello m) throws IOException {
			...
			/* First, check if there's aversion bitmap supplied. WE WILL ALWAYS HAVE a controller-provided version bitmap. */
			...
			/* If there's not a bitmap present, choose the lower of the two supported versions. */
                ...
                log.info("Negotiated down to switch OpenFlow version of {} for {} using lesser hello header algorithm.", theirVersion.toString(), channel.remoteAddress());factory = OFFactories.getFactory(theirVersion);
                OFMessageDecoder decoder = pipeline.get(OFMessageDecoder.class);
                ...
                log.info("Negotiated down to controller OpenFlowversionof {} for {} using lesser hello header algorithm.", factory.getVersion().toString(), channel.remoteAddress());
            }
    ...
    class WaitFeaturesReplyState extends OFChannelState {
        ...
        void processOFHello(OFHello m) throws IOException {
			 ...
			 * we are.
			 */
            if (true || m.getVersion().equals(factory.getVersion())) { //We don't expect a hello in this state anyway
